...
import { MODULES, getmaeveinResponse } from "@/lib/game-data";
...
export default function GamePlay() {
  ...
  useEffect(() => {
    ...
    setMessages([{ sender: "maevein", text: level.maeveinMessage }]);
  }, [moduleId, levelId]);
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      const response = getmaeveinResponse(module.id, level.id, userMsg, level.secret);
      setMessages((prev) => [...prev, { sender: "maevein", text: response }]);
      setTyping(false);
  ...
  const useHint = () => {
    ...
    setHintsUsed((h) => h + 1);
    setMessages((prev) => [...prev, { sender: "maevein", text: `💡 Hint: ${hint}` }]);
  };
    ...
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-80 glass border-r border-border/50 p-6 flex flex-col gap-4 md:min-h-screen">
        ...
        <div className="text-center">
          ...
          <h2 className="font-bold text-lg">{module.maevein}</h2>
          <p className="text-sm text-primary">Level {level.id} of {module.levels.length}</p>
        </div>
        <p className="text-sm text-muted-foreground text-center italic">"{level.maeveinMessage}"</p>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
          <AnimatePresence>
            {messages.map((msg, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, x: msg.sender === "maevein" ? -20 : 20 }}
        ...
        <div className="p-4 md:p-6 border-t border-border/50">
          <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-3">
            <input
              ...
              placeholder="Chat with the maevein..."
